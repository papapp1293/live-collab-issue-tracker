...
async function seedIssues() {
    ...
    for (const [title, description, assigned_to, status] of issues) {
        await db.query(
            `INSERT INTO issues (title, description, assigned_to, status) VALUES ($1, $2, $3, $4)
             ON CONFLICT DO NOTHING`,
            [title, description, assigned_to, status]
        );
    }

    console.log('✅ Sample issues seeded with detailed descriptions');
}
